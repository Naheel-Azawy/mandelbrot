...
C12832_LCD lcd ; // used pins: p5, p7, p6, p8, p11);
#else
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
#ifdef MBED
  lcd.pixel(px, py, iter >= maxiter ? 0 : 1);
#else
  if (to_stdout) {
    if (iter >= maxiter) {
      //putchar(' ');
...
#ifdef MBED
#define julia7885_ITER 15
#define julia7885_ZOOM 1
#else
#define julia7885_ZOOM 0
#define julia7885_ITER 100
#endif
void julia7885(bool autores, unsigned int w, unsigned int h, double step, int delay) {
  ...
  for (;;) for (double a = 0; a <= 2; a += step) {
    set_values(
      "stdout", autores, w, h,
      julia7885_ITER, julia7885_ZOOM, 0, 0, true,
      (r * cos(a * pi)),
      (r * sin(a * pi))
    );
    julia();
...
#ifdef MBED
#define brotzoom_ITER 15
#define brotzoom_MAX z <= 5
#else
#define brotzoom_ITER 2000
#define brotzoom_MAX
#endif
void brotzoom(bool autores, unsigned int w, unsigned int h, double step, int delay) {
  for (double z = 0; brotzoom_MAX; z += step) {
    set_values(
      "stdout", autores, w, h,
      brotzoom_ITER*z, z, -1.5, 0,
      false, 0, 0
    );
    mandelbrot();
    end();
...
#ifdef MBED
  brotzoom(true, -1, -1, 0.1, 0);
  julia7885(true, -1, -1, 0.01, 1);
#else
  ascii_chars = "AB:!+%$@#=";
